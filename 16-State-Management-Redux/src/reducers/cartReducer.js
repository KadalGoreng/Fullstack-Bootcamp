...
const cartReducer = (state = initState, action) => {
  ...
  if (action.type === ADD_TO_CART) {
    ...
    let found = state.addedItems.find((item) => action.id === item.id);
    if (found) {
  ...
  if (action.type === REMOVE_ITEM) {
    // Add code here
    let found = state.addedItems.find((item) => action.id === item.id);
    let delItem = state.addedItems.filter((item) => action.id !== item.id);

    return {
      ...state,
      addedItems: delItem,
      total: state.total - found.price * found.quantity,
    };
  }

  //INSIDE CART COMPONENT
  if (action.type === ADD_QUANTITY) {
    let found = state.items.find((item) => item.id === action.id);
    found.quantity += 1;
  ...
  if (action.type === SUB_QUANTITY) {
    ...
    if (found.quantity === 1) {
      let delItem = state.addedItems.filter((item) => item.id !== action.id);
